...
const typeName = t => ({single:'Un răspuns',multiple:'Răspunsuri multiple',matrix:'Yes / No',matching:'Potrivire',dropdown:'Dropdown'}[t] || t);
...
function renderSetup(){
  ...
  <div class="setup-grid"><section class="card pad"><h2 class="h2">Configurează testul</h2><p class="muted">Întrebările păstrează formularea din setul de practică; răspunsurile și explicațiile apar doar după finalizare.</p>
  <div class="controls"><div class="field"><label>Capitol</label><select id="chapter" class="select">${chapterOpts}</select></div><div class="field"><label>Număr întrebări</label><select id="count" class="select"></select></div><div class="field"><label>Ordine</label><select id="order" class="select"><option value="mixed">Amestecat</option><option value="source">În ordinea din set</option></select></div></div>
...
function renderTest(){
  ...
  app.innerHTML=`<div class="progress-card card"><div class="progress-line"><i style="width:${pct}%"></i></div><div class="progress-meta"><span>Întrebarea ${state.index+1} din ${state.questions.length}</span><span>${pct}% parcurs</span></div></div><div class="test-shell"><aside class="card nav-card"><h3>Navigare</h3><div class="nav-grid">${state.questions.map((x,i)=>`<button class="nav-q ${navStatus(x,i)}" data-i="${i}">${i+1}</button>`).join('')}</div><div class="legend"><span><i class="dot current"></i>Curentă</span><span><i class="dot answered"></i>Răspuns</span><span><i class="dot skipped"></i>Sărită</span></div></aside><section><article class="card question-card"><div class="question-top"><div class="qid">${q.code}</div><div class="badge">${typeName(q.type)}</div></div><div class="qmeta">Cap. ${q.chapter} • ${esc(q.section)} ${esc(q.sectionTitle)}</div><div class="qtitle">${esc(q.prompt)}</div>${(q.images||[]).map(x=>`<img class="qimage" src="${x.url}" alt="Imagine pentru întrebare">`).join('')}<div id="answerArea"></div><div class="question-actions"><button id="back" class="ghost" ${state.index===0?'disabled':''}>← Înapoi</button><div class="right"><button id="skip" class="ghost">Sari peste</button><button id="next" class="btn secondary">${state.index===state.questions.length-1?'Revizuiește':'Următoarea →'}</button><button id="finish" class="btn danger">Termină testul</button></div></div></article></section></div>`;
  document.querySelectorAll('.nav-q').forEach(b=>b.onclick=()=>{state.index=Number(b.dataset.i);renderTest()});
  renderAnswerArea(q);
  document.getElementById('back').onclick=()=>{if(state.index>0){state.index--;renderTest()}};
  document.getElementById('skip').onclick=()=>{state.skipped.add(q.id);delete state.answers[q.id];if(state.index<state.questions.length-1)state.index++;renderTest()};
  document.getElementById('next').onclick=()=>{if(state.index<state.questions.length-1)state.index++;else toast('Poți verifica întrebările din navigare sau finaliza testul.');renderTest()};
...
function renderAnswerArea(q){
  ...
  if(q.type==='single'||q.type==='multiple'){
    ...
    area.querySelectorAll('input').forEach(inp=>inp.onchange=()=>{state.skipped.delete(q.id);if(q.type==='single')state.answers[q.id]=inp.value;else{let a=Array.isArray(state.answers[q.id])?[...state.answers[q.id]]:[];a=inp.checked?[...new Set([...a,inp.value])]:a.filter(x=>x!==inp.value);state.answers[q.id]=a;}renderTest();});
    ...
    area.querySelectorAll('input').forEach(inp=>inp.onchange=()=>{const a=Array.isArray(state.answers[q.id])?[...state.answers[q.id]]:Array(q.statements.length).fill(null);a[Number(inp.dataset.i)]=inp.value==='true';state.answers[q.id]=a;state.skipped.delete(q.id);renderTest();});
